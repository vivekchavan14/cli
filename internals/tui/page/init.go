...
func (i *initPage) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	if i.form.State == huh.StateCompleted {
		...
		configPath := filepath.Join(os.Getenv("HOME"), ".omnitrix.yaml")
...
func NewInitPage() tea.Model {
	...
	apiKeysGroup := huh.NewGroup(
		huh.NewNote().
			Title("API Keys").
			Description("You need to provide at least one API key to use omnitrix"),
	...
	generalGroup := huh.NewGroup(
		huh.NewNote().
			...
			Description("Configure general omnitrix settings"),

			huh.NewInput().
				Title("Data Directory").
				Placeholder(".omnitrix").
				Key("data_dir").
	...
	return layout.NewSinglePane(
		...
		layout.WithSignlePaneBorderText(
			map[layout.BorderPosition]string{
				layout.TopMiddleBorder: "Welcome to omnitrix - Initial Setup",
			},
		),
	)
}
