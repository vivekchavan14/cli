...
import (
	...
	toolsPkg "github.com/omnitrix-sh/cli/internal/llm/tools"
...
func newCopilotClient(opts providerClientOptions) CopilotClient {
	...
	if copilotOpts.bearerToken != "" {
		...
		if githubToken == "" {
			logging.Error("GitHub token is required for Copilot provider. Set GITHUB_TOKEN environment variable, configure it in omnitrix.json, or ensure GitHub CLI/Copilot is properly authenticated.")
	...
	openaiClientOptions = append(openaiClientOptions,
		option.WithHeader("Editor-Version", "Omnitrix/1.0"),
		option.WithHeader("Editor-Plugin-Version", "Omnitrix/1.0"),
